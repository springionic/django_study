...
- 1:N：OneToMany
    ...
    - 比如，一个学校可以有很多个老师，一个老师只能有一个学校
    - 使用
        - 使用ForeignKey
        - 在多的那一边，比如上边的例子是在Teacher的表格里进行定义
    - add：
        - 跟一对一方法类似，通过create或new添加
        - create: 把属性都填满，不需要手动保存
        - new：可以属性或者参数为空，必须用save保存
    - query：
        - 以学校和老师的例子为准，则通过增加的关系属性，直接使用
        - 例如：查找t老师是哪个学校的
        - 反查：
            - 有学校，查学校的所有老师，则需要在老师模型名称的小写下直接加下划线set.all()，  
- N:N：ManyToMany
    - 表示任意一个表的数据都可以拥有对方表格多项数据，反之亦然
    - 比如典型例子就是 学生和老师的对应关系
    - 使用上：在任意的一方，使用ManyToMany定义，只需定义一方
    - add：
...
## 模板系统(项目继续使用tlxy_db)
- 模板：一组相同或者相似的页面，在需要个性化的地方进行留白，需要的时候用数据填充就可使用
- 步骤：
    1. 在settings进行设置：TEMPLATES
    2. 在templates文件夹中下编写模板并调用
### 模板-变量
- 变量的表示方法：{{ var_name }}
- 在系统调用模板的时候，会用相应的数据查找相应的变量名称，如果能找到则填充，或者叫渲染，否则，跳过
- 案例 two.html
### 模板-标签
- for标签:  {% for ... in .. %}
    - 用法：
...
## session(开始使用teacher_session)
- 为了对应HTTP协议的无状态性
- 用来保存用户比较敏感的信息
- 属于request 的一个属性
- 常用操作
    - request.session.get(key, defaultValue)
    - request.session.clear()  清除全部值
